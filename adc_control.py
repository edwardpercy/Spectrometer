...
draw.text((((width/2) - (9*12)),0), "Photo Spectrometry", fill=BLACK, font = font)
...
while(True):
	...
	if state == GPIO.HIGH:
		count = 0
		results = []
		draw.text((((width/2) - (6*12)),20), "Scanning ...", fill=BLACK, font = font)
		papirus.display(image)
		...
		papirus.clear()
		draw.text((((width/2) - (9*12)),8), "Spectral Results", fill=BLACK, font = font)
